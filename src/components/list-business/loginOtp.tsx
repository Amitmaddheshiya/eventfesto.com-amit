...
const Index: React.FC<LoginOtpProps> = ({ email, type }) => {
  ...
  const handleOtp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post(
        `${baseUrl}auth/verify-vendor-account`,
        inputs
      );
  ...
  const vendorResentOtp = async () => {
    // Include email and type in the data object
    try {
  ...
  return (
    <div className="w-full fixed inset-0 flex justify-center items-center bg-gray-800 bg-opacity-50 z-50">
      <div className="w-10/12 bg-white  rounded-md shadow-md md:w-3/12 h-fit border-4 border-cyan-500">
        <section className=" flex justify-center items-center h-fit bg-deepskyblue">
          <div className="flex flex-col justify-center items-center w-full h-fit bg-white p-2 md:p-6 rounded-lg shadow-md">
            <form onSubmit={handleOtp} className="">
              <div className="text-2xl font-bold mb-2 text-center">OTP</div>
              ...
              <div className="text-2xl font-bold mb-2 mt-2 text-center">
                Verification Code
              </div>
              <p className="text-gray-500 mb-2 text-center">
                We have sent a verification code to your email id.
              </p>
              <div id="inputs" className="flex justify-center">
                <input
                  type="text"
                  maxLength={4}
                  value={inputs.otp}
                  onChange={handleChange}
                  className="w-full h-8 text-center outline-none border-b-2 border-gray-300 mr-2"
                />
              </div>
              <button
                type="submit"
                className="w-full mt-4 py-3 px-4 bg-blue-500 text-white rounded-md cursor-pointer"
              >
                Confirm
              </button>
            </form>
