...
import CategoryCard from "../category-card"; // import your CategoryCard component here
import EInvitationCard from "../e-invitation-card"; // import your EInvitationFilter component here
...
const Index: React.FC = () => {
  ...
  const [selectedFilter, setSelectedFilter] = useState<"e-invitation" | "card">(
    "e-invitation"
  );
  ...
  return (
    <div>
      <div className="w-full flex md:pl-14 justify-start text-sm md:text-xl mt-8 mb-8">
        <button
          className={`mx-1 my-2 px-1 py-2 text-black ${selectedFilter === "e-invitation" ? "rounded-md px-2 bg-black text-white" : ""}`}
          onClick={() => setSelectedFilter("e-invitation")}
        >
          E-Invitation
        </button>
        <button
          className={`mx-1 my-2 px-1 py-2  text-black ${selectedFilter === "card" ? "rounded-md px-2 bg-black text-white" : ""}`}
      ...
      {selectedFilter === "e-invitation" ? (
        <EInvitationCard /> // Render your EInvitationFilter component
      ) : (
        <CategoryCard /> // Render your CategoryCard component with the images array
      )}
    </div>
  );
};
