...
interface ImageUploadProps {
  onDone: (files: (File & { foodMenuImage: string })[]) => void;
  arrowIcon: () => void;
}
...
const ImageUpload: React.FC<ImageUploadProps> = ({ onDone, arrowIcon }) => {
  ...
  useEffect(() => {
    const storedFiles = localStorage.getItem("selectedFilesfoodMenuImage");
    if (storedFiles) {
      try {
        ...
        const filesWithfoodMenuImage = parsedFiles.map((fileData) => {
          // Convert Blob to File
          const blob = new Blob([], { type: fileData.type });
          const file = new File([blob], fileData.name, { type: fileData.type });
          return { ...file, foodMenuImage: fileData.foodMenuImage };
        });
        setSelectedFiles(filesWithfoodMenuImage);
        setUploadStatus(filesWithfoodMenuImage.map(() => "success"));
      } catch (error) {
    ...
    return () => {
      selectedFiles.forEach((file) => URL.revokeObjectURL(file.foodMenuImage));
    };
  }, []);
  ...
  useEffect(() => {
    if (selectedFiles.length > 0) {
      const filesToStore = selectedFiles.map((file) => ({
        ...
        type: file.type,
        size: file.size,
        foodMenuImage: file.foodMenuImage,
      }));
      localStorage.setItem(
        "selectedFilesfoodMenuImage",
        JSON.stringify(filesToStore)
      );
    } else {
      localStorage.removeItem("selectedFilesfoodMenuImage");
    }
  }, [selectedFiles]);
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      const newFiles = Array.from(event.target.files).map((file) => ({
        ...file,
        foodMenuImage: URL.createObjectURL(file),
      })) as (File & { foodMenuImage: string })[];
      setSelectedFiles((foodMenuImageFiles) => [
        ...foodMenuImageFiles,
        ...newFiles,
      ]);
      setUploadStatus((foodMenuImageStatus) => [
        ...foodMenuImageStatus,
        ...newFiles.map(() => "idle"),
      ]);
      event.target.value = "";
  ...
  const handleRemoveFile = (index: number) => {
    URL.revokeObjectURL(selectedFiles[index].foodMenuImage);
  ...
  return (
    ...
    >
      <div className="bg-gray-50 py-2 px-6 text-xl flex items-center">
        ...
        <h1 className="md:text-xl text-sm font-bold">Upload Images to Menu</h1>
      </div>
        ...
        {selectedFiles.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
            {selectedFiles.map((file, index) => (
              <div key={file.foodMenuImage} className="relative">
